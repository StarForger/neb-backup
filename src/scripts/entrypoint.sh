...
function entrypoint_run() { 
  ...
  # Assert backup file
  assert_file_exists "${config_dir}/${NEB_BACKUP_TYPE,,}.sh" "${NEB_BACKUP_TYPE} is not a valid backup type."

  # Source backup file
  . "${config_dir}/${NEB_CONFIG_NAME,,}.sh"  

  ${NEB_CONFIG_NAME,,}_run "init"

  sleep ${NEB_BACKUP_DELAY:-60}

  while true; do 
    # TODO hook before backup
    ${NEB_CONFIG_NAME,,}_run "backup"
    # TODO hook after backup

    ${NEB_CONFIG_NAME,,}_run "prune"
